...
export class FlutterwaveProvider implements PaymentProvider {
  ...
  async createTransfer(input: MethodInput): Promise<TransferInstructions> {
    ...
    const body = await res.json().catch(() => ({}));
    if (!res.ok || body?.status !== "success") {
      throw new Error(body?.message ?? `Flutterwave bank-transfer charge failed (${res.status})`);
    }

    // A fresh, single-use NUBAN is minted per charge — this *is* the dynamic
    // virtual account, generated by Flutterwave for this exact transaction.
    const a = body?.data?.meta?.authorization ?? body?.meta?.authorization ?? {};
      if (!a.transfer_account || !a.transfer_bank) {
      throw new Error("Flutterwave did not return a virtual account for this charge");
      }
    return {
      method: "bank_transfer",
      bankName: a.transfer_bank ,
      accountNumber: a.transfer_account,
      accountName: a.transfer_note ?? "SPURS PAY",
      amount: input.amount,
      currency: input.currency,
      expiresAt: a.account_expiration
        ? new Date(a.account_expiration).toISOString()
